...
const DISTRICTS: TownDistrict[] = [
  {
    id: "gate-watch",
    label: "Gate & Watch",
    purpose: "Arrival, orientation, guards, and the southern threshold.",
    minX: -42, maxX: 42, minZ: 20, maxZ: 60,
    anchor: { x: 0, z: 38 }
  },
  {
    id: "main-street",
    label: "Main Street",
    purpose: "Primary civic and commercial circulation spine.",
    minX: -18, maxX: 18, minZ: 48, maxZ: 190,
    anchor: { x: 0, z: 112 }
  },
  {
    id: "town-center",
    label: "Town Center",
    purpose: "Central landmark, well, green, and social orientation point.",
    minX: -30, maxX: 30, minZ: 86, maxZ: 136,
    anchor: { x: 0, z: 108 }
  },
  {
    id: "market",
    label: "Market Square",
    purpose: "Pedestrian trading court connected to Main Street.",
    minX: -84, maxX: -24, minZ: 82, maxZ: 148,
    anchor: { x: -52, z: 116 }
  },
  {
    id: "guild-court",
    label: "Guild Court",
    purpose: "Guild Hall, contracts, adventurer traffic, and civic authority.",
    minX: 30, maxX: 86, minZ: 100, maxZ: 170,
    anchor: { x: 62, z: 132 }
  },
  {
    id: "residential-lane",
    label: "Residential Lane",
    purpose: "Homes, quieter circulation, and lived-in town texture.",
    minX: -92, maxX: -18, minZ: 150, maxZ: 218,
    anchor: { x: -58, z: 184 }
  },
  {
    id: "service-yard",
    label: "Service Yard",
    purpose: "Workshops, storage, carts, deliveries, and practical industry.",
    minX: 50, maxX: 96, minZ: 62, maxZ: 142,
    anchor: { x: 76, z: 102 }
  },
  {
    id: "frontier-exit",
    label: "Frontier Supply Row & Exit",
    purpose: "Expedition preparation and the northern frontier transition.",
    minX: -34, maxX: 38, minZ: 174, maxZ: 224,
    anchor: { x: 0, z: 204 }
  }
];
...
const CORRIDORS: WalkCorridor[] = [
  {
    id: "gate-to-frontier",
    label: "Gate → Town Center → Frontier Exit",
    halfWidth: 7,
    points: [
      { x: 0, z: 20 }, { x: -1, z: 50 }, { x: 3, z: 78 },
      { x: 0, z: 108 }, { x: 4, z: 142 }, { x: 0, z: 176 }, { x: 0, z: 224 }
    ]
  },
  {
    id: "market-loop",
    ...
    halfWidth: 5,
    points: [
      { x: 0, z: 90 }, { x: -22, z: 98 }, { x: -52, z: 116 },
      { x: -62, z: 140 }, { x: -32, z: 154 }, { x: 0, z: 150 }
    ]
  },
  {
    id: "guild-loop",
    ...
    halfWidth: 5,
    points: [
      { x: 0, z: 104 }, { x: 24, z: 110 }, { x: 58, z: 128 },
      { x: 68, z: 152 }, { x: 36, z: 166 }, { x: 0, z: 160 }
    ]
  },
  {
    id: "residential-loop",
    ...
    halfWidth: 4.5,
    points: [
      { x: 0, z: 154 }, { x: -28, z: 166 }, { x: -62, z: 184 },
      { x: -72, z: 204 }, { x: -36, z: 214 }, { x: 0, z: 198 }
    ]
  }
];
...
const SIGHTLINES: Sightline[] = [
  {
    id: "gate-to-center",
    label: "Gate frames the Town Center landmark",
    from: { x: 0, z: 24 },
    to: { x: 0, z: 108 },
    halfWidth: 3.5
  },
  {
    ...
    label: "Town Center reveals the Frontier Exit",
    from: { x: 0, z: 108 },
    to: { x: 0, z: 216 },
    halfWidth: 3.5
  }
...
const pointInPolygon = (point: TownPoint, polygon: TownPoint[]): boolean => {
  ...
  for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
    ...
    const intersects = ((a.z > point.z) !== (b.z > point.z))
      && point.x < (b.x - a.x) * (point.z - a.z) / ((b.z - a.z) || 0.0001) + a.x;
...
const districtOverlap = (a: TownDistrict, b: TownDistrict): boolean => (
  a.minX < b.maxX && a.maxX > b.minX && a.minZ < b.maxZ && a.maxZ > b.minZ
);

const segmentDistance = (point: TownPoint, start: TownPoint, end: TownPoint): number => {
...
export class CaelusTownBoundaryDirector {
  ...
  private audit: TownBoundaryAudit;
  ...
  constructor(game: any) {
    ...
    console.info(`[Caelus Boundary] ${DISTRICTS.length} districts, ${CORRIDORS.length} corridors, pass=${this.audit.pass}.`);
  }
  ...
  private buildAudit(): TownBoundaryAudit {
    ...
    const districtArea = DISTRICTS.reduce((total, district) => (
      total + (district.maxX - district.minX) * (district.maxZ - district.minZ)
    ), 0);
    ...
    for (let left = 0; left < DISTRICTS.length; left += 1) {
      for (let right = left + 1; right < DISTRICTS.length; right += 1) {
        ...
        if (!districtOverlap(a, b)) continue;
        const permitted = a.id === "main-street" || b.id === "main-street" || a.id === "town-center" || b.id === "town-center";
        if (!permitted) districtOverlapPairs.push(`${a.id}:${b.id}`);
      }
    }
    ...
    const sightlines = SIGHTLINES.map((sightline) => {
      ...
      for (let index = 1; index < 30; index += 1) {
        const t = index / 30;
        const point = {
          x: sightline.from.x + (sightline.to.x - sightline.from.x) * t,
          z: sightline.from.z + (sightline.to.z - sightline.from.z) * t
        };
        const blocked = collisionBoxes.some((box: any) => (
          point.x > Number(box.minX) && point.x < Number(box.maxX)
          && point.z > Number(box.minZ) && point.z < Number(box.maxZ)
        ));
        if (blocked) blockedSamples += 1;
      }
      return { ...sightline, blockedSamples };
    });

    const protectedRoute = CORRIDORS[0].points;
    const protectedRouteInsideBoundary = protectedRoute.every((point) => pointInPolygon(point, FINAL_PERIMETER));
    const districtAnchorsInsideBoundary = DISTRICTS.every((district) => pointInPolygon(district.anchor, FINAL_PERIMETER));
    const foundryStartZ = 250;
    const foundrySeparation = foundryStartZ - Math.max(...zs);
  ...
  private createWorldOverlay(): void {
    const makePath = (name: string, points: TownPoint[], yOffset: number, color: any, closed = false): any => {
      ...
      const mesh = BABYLON.MeshBuilder.CreateLines(name, { points: vectors, updatable: false }, this.scene);
      mesh.color = color;
    ...
    for (const corridor of CORRIDORS) {
      makePath(`caelus-boundary-corridor-${corridor.id}`, corridor.points, 0.48, new BABYLON.Color3(0.2, 1, 0.45));
    }
    for (const sightline of SIGHTLINES) {
      makePath(`caelus-boundary-sightline-${sightline.id}`, [sightline.from, sightline.to], 0.62, new BABYLON.Color3(1, 0.85, 0.18));
    }

    for (const district of DISTRICTS) {
      const corners = [
        { x: district.minX, z: district.minZ },
        { x: district.maxX, z: district.minZ },
        { x: district.maxX, z: district.maxZ },
        { x: district.minX, z: district.maxZ }
      ];
      makePath(`caelus-boundary-district-${district.id}`, corners, 0.52, new BABYLON.Color3(0.85, 0.35, 1), true);
      const marker = BABYLON.MeshBuilder.CreateCylinder(`caelus-boundary-anchor-${district.id}`, {
        height: 3.4,
        diameter: 0.55,
        tessellation: 8
      }, this.scene);
      marker.position.set(
        district.anchor.x,
        this.world.heightAt(district.anchor.x, district.anchor.z) + 1.7,
        district.anchor.z
      );
  ...
  private createHudOverlay(): void {
    ...
    heading.innerHTML = `<strong>SET 1.2 · FINAL TOWN BOUNDARY</strong><br><span style="opacity:.72">F8 toggles boundary diagnostics</span>`;
    heading.style.marginBottom = "12px";
    overlay.append(heading);

    const stats = document.createElement("div");
    stats.style.fontSize = "12px";
    stats.style.lineHeight = "1.55";
    stats.innerHTML = [
      `Perimeter: <b>${this.audit.perimeterArea}</b> units²`,
      `Compactness: <b>${this.audit.compactness}</b>`,
      `Districts: <b>${this.audit.districts.length}</b>`,
      `Walk corridors: <b>${this.audit.corridors.length}</b>`,
      `Foundry separation: <b>${this.audit.foundrySeparation}</b> units`,
      `Dead-space ratio: <b>${this.audit.deadSpaceRatio}</b>`,
      `Milestone gate: <b style="color:${this.audit.pass ? "#66ff9c" : "#ff6d6d"}">${this.audit.pass ? "PASS" : "FAIL"}</b>`
    ].join("<br>");
    overlay.append(stats);

    const list = document.createElement("div");
    list.style.marginTop = "12px";
    list.innerHTML = this.audit.districts.map((district) => (
      `<div style="margin:8px 0;padding:8px;border-left:3px solid #d15cff;background:rgba(255,255,255,.035)">`
      + `<b>${district.label}</b><br><span style="font-size:11px;opacity:.75">${district.purpose}</span></div>`
    )).join("");
    overlay.append(list);
