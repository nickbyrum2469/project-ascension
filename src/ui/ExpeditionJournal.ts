...
interface JournalSave {
  ...
  expedition?: { activeBeacon?: string; activatedBeacons?: string[]; claimedCaches?: string[]; riftglassShards?: number; ascentCompleted?: boolean };
}
...
export class ExpeditionJournal {
    ...
    this.root.innerHTML = `
      ...
      <article class="journal-panel" role="dialog" aria-modal="true" aria-labelledby="journal-title">
        <header class="journal-header">
          ...
          <div><p>FOUNDATION ARCHIVE</p><h2 id="journal-title">Expedition Journal</h2><small>Field intelligence, route memory, and permanent floor progress.</small></div>
  ...
  private render(): void {
    ...
    this.content.innerHTML = this.activeTab === "overview" ? this.overview(save) : this.activeTab === "records" ? this.records(save) : this.network(save);
  }
  ...
  private overview(save: JournalSave): string {
    ...
    return `<section class="journal-hero"><p>ACTIVE THREAD</p><h3>${chapter[0]}</h3><span>${chapter[1]}</span></section>
      <div class="journal-stats">${this.stat("Riftglass", `${expedition.riftglassShards ?? 0} shards`, "Recovered caches")}${this.stat("Relays", `${sigils} / 3`, "Foundry synchronization")}${this.stat("Beacons", `${expedition.activatedBeacons?.length ?? 1} / 4`, "Permanent rest points")}${this.stat("Ascent", expedition.ascentCompleted ? "Recorded" : "Sealed", "Eastern threshold")}</div>
  ...
  private assessment(save: JournalSave): string {
    ...
    return "Floor One's permanent ascent route is established. Remaining work is exploration completion and upper-threshold preparation.";
  }
