...
public class MainActivityV4 extends MainActivityV3 {
    ...
    private void addV4Controls() {
        ...
        TextView spacer = new TextView(this); row.addView(spacer, new LinearLayout.LayoutParams(dp4(8), 1));
    ...
    private void beginMirror() {
        if (Build.VERSION.SDK_INT < 29) { Toast.makeText(this, "Playback capture needs Android 10 or newer.", Toast.LENGTH_LONG).show(); return; }
    ...
    private void requestCapture() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage("Android will ask for screen/audio capture permission. SyncLink configures the audio capture to match Pandora's app UID and does not save audio. Keep Pandora playing to your Bluetooth outputs while the mirror is active.")
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode != RESULT_OK || data == null) { Toast.makeText(this, "Capture permission was not granted.", Toast.LENGTH_LONG).show(); return; }
        Intent s = new Intent(this, PandoraMirrorService.class);
        s.putExtra("resultCode", resultCode); s.putExtra("resultData", data);
        if (Build.VERSION.SDK_INT >= 26) startForegroundService(s); else startService(s);
        new AlertDialog.Builder(this)
                .setTitle("Mirror started")
                .setMessage("Now play Pandora normally. Its original stream should stay on the Bluetooth output(s), while SyncLink tries to reproduce a captured copy on this phone's speaker. Watch the SyncLink notification: it will say whether Pandora audio is capturable or only silence is being returned.")
    ...
    private void openPandora4() {
        try { Intent i = getPackageManager().getLaunchIntentForPackage("com.pandora.android"); if (i != null) { startActivity(i); return; } }
        catch (Exception ignored) {}
        Toast.makeText(this, "Pandora isn't installed.", Toast.LENGTH_LONG).show();
    }

    private void openSamsungMedia() {
        try { Intent launch = getPackageManager().getLaunchIntentForPackage("com.samsung.android.mdx.quickboard"); if (launch != null) { startActivity(launch); return; } }
        catch (Exception ignored) {}
        try { startActivity(new Intent("com.android.settings.panel.action.MEDIA_OUTPUT")); return; }
        catch (Exception ignored) {}
        new AlertDialog.Builder(this)
                .setTitle("Open Samsung Media output")
                .setMessage("Swipe down from the top-right, tap Media output, and look for the small circles beside connected Bluetooth speakers. Samsung's own panel is the one that exposes Dual Audio selection.")
                .setPositiveButton("OK", null)
                .show();
    }

    private Button button4(String text, int color) {
        Button b = new Button(this); b.setText(text); b.setTextColor(Color.WHITE); b.setTextSize(10); b.setTypeface(Typeface.DEFAULT, Typeface.BOLD); b.setAllCaps(false); b.setBackground(round4(color, 13)); b.setStateListAnimator(null); return b;
    }
